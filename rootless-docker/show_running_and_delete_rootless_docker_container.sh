...
while true; do
    ...
    echo "1) EXIT"
    echo "2) DELETE a container by S.No"
    ...
    case "$CHOICE" in
        1)
            ...
            ;;
        2)
            read -rp "Enter S.No of container to DELETE: " SNO
            ENTRY="${CONTAINER_MAP[$SNO]}"

            if [ -n "$ENTRY" ]; then
                USER="${ENTRY%%:*}"
                CID="${ENTRY##*:}"
                echo "Are you sure you want to delete container $CID from user $USER? (y/n)"
                read -r CONFIRM
                if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
                    USER_ID=$(id -u "$USER")
                    export DOCKER_HOST=unix:///run/user/$USER_ID/docker.sock
                    sudo -u "$USER" docker rm -f "$CID"
                    echo "Container $CID deleted."
                else
                    echo "Deletion cancelled."
                fi
            else
                echo "Invalid S.No entered."
            fi
            ;;
        *)
