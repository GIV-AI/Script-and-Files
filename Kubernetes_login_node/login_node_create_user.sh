...
Folder_send_to_login_user="/home/jain-dgx/Scripts_and_Files/Kubernetes_login_node/script_file_send_to_login_user/"  # Corrected variable assignment
...
while true; do
    read -p "Enter 141GB GPU (0-9): " GPU_141GB
    validate_number "$GPU_141GB" 0 9 && break
done

while true; do
    read -p "Enter 71GB GPU (0-9): " GPU_71GB
    validate_number "$GPU_71GB" 0 9 && break
done

while true; do
    read -p "Enter 35GB GPU (0-9): " GPU_35GB
    validate_number "$GPU_35GB" 0 9 && break
done
 
while true; do
    read -p "Enter 18GB GPU (0-9): " GPU_18GB
    validate_number "$GPU_18GB" 0 9 && break
done
...
spec:
  hard:
    cpu: "$CPU"
    memory: "${MEMORY}Gi"
    requests.nvidia.com/gpu: "$GPU_141GB"
    requests.nvidia.com/mig-3g.71gb: "$GPU_71GB"
    requests.nvidia.com/mig-2g.35gb: "$GPU_35GB"
    requests.nvidia.com/mig-1g.18gb: "$GPU_18GB"
    pods: "$PODS"
EOF
...
CLUSTER_SERVER="https://192.168.1.248:6443"
...
scp config_user.yaml "$USERNAME"@192.168.1.250:/home/$USERNAME/.kube/config
scp -r "$Folder_send_to_login_user"/* "$USERNAME"@192.168.1.250:/home/$USERNAME/
